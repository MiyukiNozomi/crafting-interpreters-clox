...
static void emitLoop(int loopStart) {
    ...
    // The + 2 is to take into account the size of the OP_LOOP instruction’s own operands
...
static void patchJump(int offset) {
    // -2はjump先の値(jump先は16bitで表しており、1命令8bitなので2つ分引く必要がある。)
...
static int addUpvalue(Compiler* compiler, uint8_t index, bool isLocal) {
    ...
    if (upvalueCount == UINT8_COUNT) {
        error("Too many closuer variables in closure.");
        return 0;
    }
...
static void forStatement() {
    ...
    if (!match(TOKEN_RIGHT_PAREN)) {
        ...
        int incrementsStart = currentChunk()->count;
        expression();
        emitByte(OP_POP);
        consume(TOKEN_RIGHT_PAREN, "Expect ')' after for clauses.");

        emitLoop(loopStart);
        loopStart = incrementsStart;
        patchJump(bodyJump);
    }
